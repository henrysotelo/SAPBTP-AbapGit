CLASS zcl_http_data_load_8712 DEFINITION
  PUBLIC
  CREATE PUBLIC .
  ...
  PRIVATE SECTION.
    ...
    METHODS:
      get_html
        RETURNING VALUE(ui_html) TYPE string.
ENDCLASS.

CLASS zcl_http_data_load_8712 IMPLEMENTATION.
...
* Check table name is valid.
        IF tablename IS INITIAL.
          response->set_status(
              i_code = if_web_http_status=>bad_request i_reason = |Table name { tablename } not valid or does not exist| ).
        ...
        IF fileext <> 'json'.
          response->set_status(
              i_code = if_web_http_status=>bad_request i_reason = 'File type not supported' ).
